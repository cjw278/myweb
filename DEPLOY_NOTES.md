...
部署到宝塔面板后，浏览器访问 `https://yvci.site` 出现大量混合内容错误：

```
Mixed Content: The page at 'https://yvci.site/' was loaded over HTTPS,
but requested an insecure resource 'http://yvci.site:8000/api/posts/count?status=published'.
This request has been blocked; the content must be served over HTTPS.
```

关键现象：
- 编译产物里**完全没有** `http://yvci.site:8000` 或 `localhost:8000`
- `curl https://yvci.site/api/health` 返回 `{"status":"ok"}`，Nginx 反代正常
...
4. 部署后用 `curl -s https://yvci.site/ | grep buildId` 确认返回的是最新构建
